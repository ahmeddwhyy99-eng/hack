...
export default function WalletPage({ id }: { id: string }) {
  ...
  return <Shell wallet title="Review this request" subtitle="You decide what to share with EventPass.">{!context && !error ? <Loading/> : null}{context && <Card><div className="identity-row"><UserRound size={22} aria-hidden="true"/><div><small>Demo identity</small><strong>{context.user.displayName}</strong></div></div><div className="request-heading"><Ticket size={22} aria-hidden="true"/><h2>{context.relyingParty}</h2></div><p>{context.requestText}</p><div className="disclosure shared"><div className="section-label"><Eye size={20} aria-hidden="true"/><h3>What will be shared</h3></div><ul><li>{context.sharedText}</li></ul></div><div className="disclosure"><div className="section-label"><EyeOff size={20} aria-hidden="true"/><h3>What will NOT be shared</h3></div><ul>{context.notShared.map(item => <li key={item}>{item}</li>)}</ul></div><Separator/><p className="consent">{context.consentText}</p>{error ? <ErrorNotice error={error} retry={() => void read()}/> : null}{context.status !== 'pending' ? <><p>This request already has a decision.</p><Button asChild className="full"><Link href={`/event/result/${id}`}>View result <ArrowUpRight size={18}/></Link></Button></> : <div className="actions"><Button variant="outline" disabled={Boolean(busy) || mustRefresh} onClick={() => void decide('reject')}>{busy === 'reject' ? 'Rejecting…' : 'Reject'}</Button><Button disabled={Boolean(busy) || mustRefresh} onClick={() => void decide('approve')}>{busy === 'approve' ? 'Approving…' : 'Approve'}</Button></div>}{busy && <p role="status" className="helper">Saving your decision…</p>}</Card>}{!context && error ? <ErrorNotice error={error} retry={() => void read()}/> : null}</Shell>
}
